...
1-checkjobs				template 	slurm script for running check_cm_jobs with "asarray" arg
...
align_TSA_and_replace.py		within setup	fits ligand from existing TS structure into initialopt 
							  to create tsguess.pdb
...
cmsetup-new-tsguess-from-xtb.sh		set up calcs	sets up tsconstrained calcs with new tsguess structure 
							  made from xtb tsopts
...
cm-tsguess-orcaxtb.sh			within setup	creates tsguess.pdb and tsconstrained inp with 
							  align_TSA_and_replace.py and write_input.py 
							  (always uses Atsu's old DFT/xtal TS)
collect_cm_results.py			check/analyse   collects results for completed models
copy-over-done-models.sh		set up calcs	copies over files for models Domi has already done to avoid
							  repeating calculations
count_tsopt_types.sh			check/analyse   counts how many tsopts have been successfully obtained with 
							each approach and how many have been excluded
...
get_fg_frames.py			check/analyse   extracts list of frames from batch RIN output in which given 
							  functional group has contacts with ligand
orcaxtb_intmp.txt			template	input template for preparing orca xtb calcs with write_input.py
plot_CM_results.ipynb			notebook	jupyter notebook for looking at own models' data, 
							  contains structure/energy checks
...
res_13-ts-new-02-out.pdb		template	Atsu's old TS used for first ts guesses (all active sites)
...
if you need to do calcs on X-ray crystal structure models/using Gaussian or create new ts guess for one struc using 
a manually specified old ts, there are alternative versions of align_TSA_and_replace.py and cm-tsguess-orcaxtb.sh 
in ../dwappett/ that can handle those
...
fix-failed-directopts.sh	restarts direct tsopts that were set up incorrectly (before mid-April 2026)
resize-chunks.py		reorganises existing fxxxxx-fxxxxx dirs into groups of 250 models. 
				  when making new models, bigger "chunk" size should be selected from the start
plot_all_CM_together.ipynb	plots all active sites' results together, replaced by CM_MD-to-QM_paper.ipynb 
